...
After updating your dependencies, whether you upgrade, add, or remove a dependency, be sure to run
the `flatpakGradleGenerator` task to ensure the Flatpak builds with the correct dependencies. In
other words, run the following command:

```bash
./gradlew flatpakGradleGenerator
```

**Do not modify `flatpak-sources.json` directly! It is autogenerated and will be overwritten on the
next run**.
